...
% Last modified: 16-Oct-2018    
    ...
    methods (Static)
        function main()
            ...
            disp(trD);
            disp(tstD);
            model = svmtrain(trLbs, trIds', ['-c 10 -g 0.1']);
            %t = zeros()
            disp(size(tstIds));
            [predicted_label] = svmpredict(tof', tstIds, model , ['libsvm_options']);
        end
        ...
        function bowCs = learnDictionary(scales, normH, normW)
            ...
            % We set to a small number here to speed up the process. 
        ...
        function D = cmpFeatVecs(imIds, scales, normH, normW, bowCs)
            ...
            for i=1:n
                ...
                feat = histogram(bowIds, 1:size(bowCs,2));
                D{i} = feat(:);
            end
            D = cat(2, D{:});
            D = double(D);
